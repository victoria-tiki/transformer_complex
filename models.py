...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inputs, encoder_outputs, mask=None, additional_mask=None, return_weights=False):
        ...
        attention_output_2, attn_weights_2 = self.attention_2(attention_output_1, encoder_outputs, encoder_outputs, key_padding_mask=None, average_attn_weights=False)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, encoder_inputs, decoder_inputs, encoder_is_precomputed=False, return_mask=False, return_weights=False, imag_mask=None):
        if not encoder_is_precomputed:
            encoder_inputs_real = encoder_inputs[:, :, 0].unsqueeze(-1)
            encoder_inputs_imag = encoder_inputs[:, :, 1].unsqueeze(-1)
            ...
            assert imag_mask is not None, "imag_mask must be provided if encoder_is_precomputed=True"
            encoder_outputs = encoder_inputs
    
        decoder_inputs_real = decoder_inputs[:, :, 0].unsqueeze(-1)
        decoder_inputs_imag = decoder_inputs[:, :, 1].unsqueeze(-1)
        ...
        if self.return_mask or return_mask:
